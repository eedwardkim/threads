...
export default async function Page({ searchParams }: { searchParams: Promise<{ chat?: string; thread?: string }> }) {
  ...
  if (!user.isGuest) await seedIfNeeded(repository);
  ...
  return <ChatApp initialData={library} initialThread={initialThread} user={{ id: user.id, email: user.email, isGuest: user.isGuest, guestExpiresAt: user.guestExpiresAt }} providerStatus={getProviderStatus()} missingChat={missingChat} />;
}
