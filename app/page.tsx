...
export default async function Page({ searchParams }: { searchParams: Promise<{ chat?: string; thread?: string }> }) {
  ...
  const { repository } = dataFor(user.id, undefined, user.isGuest);
  const demoEnabled = user.isGuest ? false : await repository.demoEnabled();
  ...
  return <ChatApp initialData={library} initialThread={initialThread} user={{ id: user.id, email: user.email, isGuest: user.isGuest, guestExpiresAt: user.guestExpiresAt }} providerStatus={getProviderStatus()} missingChat={missingChat} demoEnabled={demoEnabled} />;
}
